...
4. Set the command to run the tests:
   ...
   ```sh
   ai-tdd config set RUN_TESTS "npm run test"
   ```
...
You pay for your own requests to OpenAI API. AI-TDD uses latest GPT model by default, check it's [pricing](https://openai.com/pricing). Maximum response tokens are set to 2000, you can adjust it via `ait config set maxTokens=<number>`.
